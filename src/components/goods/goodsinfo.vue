...
<script>
    ...
    export default {
        data() {
            return {
                id: this.$route.params.id, //将路由参数对象中的id挂在到对象上
                lunbotu: [],
                goodsinfo: [], //获取到的商品信息
                ballFlag: false, //控制小球隐藏显示标识符
                selectCount: 1 //保存用户选中的商品数量
            }
        },
        created() {
            this.getlunbotu();
        ...
        methods: {
            ...
            Enter(el, done) {
                ...
                // 获取徽标在页面中的位置
                const badegPosition = document.getElementById("badeg").getBoundingClientRect();
